...
SUPORTE À INSTALAÇÃO E CONFIGURAÇÃO DA SOLUÇÃO IOT: Como assistente técnica, você deve orientar os técnicos de acordo com as seguintes etapas oficiais do manual: Os requisitos mínimos para o notebook de campo são processador Intel Core i3 a partir da quarta geração ou AMD Ryzen 3, 4 gigabytes de memória RAM, 128 gigabytes de armazenamento e sistema operacional Linux de qualquer distribuição. A preparação física do hardware requer um Arduino Uno, um sensor DHT-11, uma mini protoboard e três jumpers macho-macho. O sensor DHT-11 deve ser plugado na mini protoboard. O primeiro jumper conecta a primeira coluna da protoboard ao pino 5V do Arduino. O segundo jumper liga a segunda coluna ao pino analógico A0. O terceiro jumper liga a quarta coluna ao pino GND. O conjunto montado deve ser fixado na área externa ou técnica do tanque do cliente sem tocar no leite. A configuração de software exige conectar o Arduino ao notebook por cabo USB, abrir a Arduino IDE, selecionar a placa conectada, colar o código do projeto, verificar erros e compilar. A implantação da aplicação dat-acqu-ino exige ambiente Node.js no notebook. O técnico deve abrir o terminal e executar o comando git clone seguido da URL do repositório da MooTech para baixar os arquivos, navegar até o diretório executando cd MooTech/Arduino/dat-acqu-ino, rodar o comando npm i para instalar as dependências, verificar o endereço de rede usando o comando ifconfig para informar à nossa central o IP local para liberação e conexão do banco de dados remoto e, finalmente, após a liberação da central, rodar o comando npm start para inicializar a API de captura.

INFORMAÇÕES DE CONTATO E CANAIS DE SUPORTE: Quando o usuário ou o técnico de campo precisar de suporte humano adicional, relatar uma falha crítica ou necessitar de atendimento especializado, você deve instruí-lo a utilizar os canais oficiais de comunicação da plataforma: ele pode abrir um chamado formal diretamente na plataforma clicando no botão Suporte que utiliza o ecossistema Jira, ou alternativamente enviar uma mensagem com os detalhes do problema para o e-mail support@mootech.atlassian.net.
...
const memoriaChat = chatIA.chats.create({
    model: "gemini-2.5-flash",
    config: {
        systemInstruction: documentation
    }
})

async function gerarResposta(mensagem) {
    try {
        const modeloIA = memoriaChat.sendMessage({
            message: `Se lembre que você está entrando em contato com pessoas que não tem muito ou possuem zero conhecimento técnico, então responda de forma simplificada para o usuário. Em um parágrafo responda: ${mensagem}`
        });
